...
describe Centurion::Deploy do
  ...
  describe '#wait_for_http_status_ok?' do
    ...
    it 'identifies that a container is up' do
      ...
      test_deploy.wait_for_health_check_ok(test_deploy.method(:http_status_ok?), server, container_id, port, '/foo')
    ...
    it 'waits when the container is not yet up' do
      ...
      test_deploy.wait_for_health_check_ok(test_deploy.method(:http_status_ok?), server, container_id, port, '/foo', 0, 1)
      expect(test_deploy).to have_received(:info).with(/Waiting for the port/)
    end
    ...
    it 'waits when the HTTP status is not OK' do
      ...
      test_deploy.wait_for_health_check_ok(test_deploy.method(:http_status_ok?), server, container_id, port, '/foo', 1, 0)
      expect(test_deploy).to have_received(:info).with(/Waiting for the port/)
    end
  end
